...
def process_chunk(chunk, chunk_idx, X_train_raw, X_test_raw, y_train, y_test):
    """Process a single chunk of combinations."""
    print(f"Processing chunk {chunk_idx + 1}...")
    chunk_results = []
    for combo in chunk:
        X_train, X_test = process_combination(X_train_raw, X_test_raw, combo['features'], combo['name'])
        if X_train is not None and X_test is not None:
            chunk_results.extend(train_and_evaluate(combo, X_train, X_test, y_train, y_test))
            # Release memory after processing each combination
            del X_train, X_test
            gc.collect()
    # Save intermediate results for the current chunk
    chunk_report_path = f"{DATA_DIR}model_performance_chunk_{chunk_idx + 1}.csv"
    pd.DataFrame(chunk_results).to_csv(chunk_report_path, index=False)
    print(f"Chunk {chunk_idx + 1} results saved to {chunk_report_path}")
    return chunk_report_path
...
if __name__ == "__main__":
    ...
    n_jobs = 4  # Number of parallel jobs (adjust based on system resources)
    chunk_report_paths = Parallel(n_jobs=n_jobs)(
        delayed(process_chunk)(chunk, chunk_idx, X_train_raw, X_test_raw, y_train, y_test)
        for chunk_idx, chunk in enumerate(combination_chunks)
    )

    # Combine all chunk results into the final report
    all_results = []
    for chunk_report_path in chunk_report_paths:
        chunk_data = pd.read_csv(chunk_report_path)
